appcan.define("slider", function($, exports, module) {
    ...
    SliderView.prototype = {
        ...
        drag:function(d,flag){
            var self = this;
            if(flag=="last"){
                if(Math.abs(d)>=self.ele.offset().width/2){
                    return;
                }
                if(d>0){
                   self.container.children(":first-child").css("background-image","");
                }else{
                   self.container.children(":last-child").css("background-image","");
                }
            }
        ...
        set:function(data){
            ...
            self.ele.off("swipeMoveLeft").on("swipeMoveLeft",function(evt){
                 ...
                 if(!self.option.circleSlide&&self.option.index==self.option.itemCount-1){
                     self.drag(-evt._args.dx,"last"); 
                 }else{
            ...
            self.ele.off("swipeMoveRight").on("swipeMoveRight",function(evt){
                ...
                if(!self.option.circleSlide&&self.option.index==0){
                    self.drag(evt._args.dx,"last");
                    return;
                }else{
                    self.drag(evt._args.dx,"");  
            ...
            self.ele.off("swipeLeft").on("swipeLeft",function(evt){
                if(self.option.index<self.option.itemCount){
                    if((self.option.index==self.option.itemCount-1)&&!self.option.circleSlide){
                        self._moveTo(self.option.index);
                    }else{
                        self._moveTo(++self.option.index);
                        self.autoMove(self.option.auto);
            ...
            self.ele.off("swipeRight").on("swipeRight",function(evt){
                if(self.option.index>=0){
                    if(self.option.index==0&&!self.option.circleSlide){
                       self._moveTo(self.option.index);
                    }else{
                        self._moveTo(--self.option.index);
                        self.autoMove(self.option.auto);
