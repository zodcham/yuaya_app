...
function loadData() {
    ...
    if(userInfo.isActiveValidity==0 && userInfo.type!=2){
        $("#vip_status").html("普通会员，点击成为VIP会员");
    }
    else{
        if(userInfo.type!=2) {
            $("#vip_status").html("VIP会员");
        }
        else{
            $("#vip_status").html("月芽学生账户");
        }
    }
...
//VIP
appcan.button("#btnVIP", "btn-act", function() {
    appcan.window.open("vip", "vip/vip.html", 5);
})

//我的订单
appcan.button("#btnOrderList", "btn-act", function() {
    appcan.window.open("order_list", "vip/order_list.html", 5);
})
