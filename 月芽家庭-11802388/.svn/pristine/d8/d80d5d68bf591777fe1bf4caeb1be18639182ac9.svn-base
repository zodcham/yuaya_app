...
appcan.ready(function() {
    ...
    AddLogContent(userInfo.id, logKeys.ClubThreadDetail, {"threadId":id});
...
function loadThreads(){
    ...
    var fok=function(data){
        ...
        if(userInfo.id == data.userid || userInfo.id=='9d0d22aba39848ce84966f8faad4b192'){
            $("#threads_del").click(function(){
                del_threads(data.tid, data.userid);
            });
            $("#threads_edit").click(function(){
                edit_threads(data.tid, data.userid);
            });
        ...
        $.each(list,function(index,value) {
            ...
            htm=htm.replace( "del_posts('yypid','yytid')", "del_posts('"+ value["pid"] +"','"+ value["tid"] +"', '"+ value["userid"] +"')");
            ...
            appcan.logs(value["imglist"])

            if(value["imglist"].length>0)
            {
                var imglist_post=value["imglist"];
                $.each(imglist_post,function(index, v) {
                    imghtml+='<img src="'+ (_SERVER_ADDRESS + v) +'" class="contentimg">';
                })
            }

            if(userInfo.id == userid || userInfo.id=='9d0d22aba39848ce84966f8faad4b192'){
            ...
            if(value["postlist"].length>0){
                ...
                $.each(floor,function(index, v) {
                    ...
                    if(v.userid==userInfo.id || userInfo.id=='9d0d22aba39848ce84966f8faad4b192'){
                        postshtml+='<input type="button" style="width:2.5em; border-width:0; text-align:center; font-size:1em;  background-color:#00A0E9 ;color:#fff;border-radius: 0.3em;margin-left:1.5em;"  onclick="edit_posts(\''+v.pid+'\',\''+v.tid+'\')"  value="修改">';
                        postshtml+='<input type="button" style="width:2.5em; border-width:0; text-align:center; font-size:1em;  background-color:#F16412 ;color:#fff;border-radius: 0.3em;margin-left:1.5em;"  onclick="del_posts(\''+v.pid+'\',\''+v.tid+'\',\''+v.userid+'\')"  value="清除">';
                    }
                    postshtml+='</p>';
                })
            ...
            if(value["postlist"].length>0){
                ...
                $.each(floor,function(index, v) {
                    ...
                    if(v.userid==userInfo.id || userInfo.id=='9d0d22aba39848ce84966f8faad4b192'){
                        ...
                        postshtml+='<input type="button" style="width:2.5em; border-width:0; text-align:center; font-size:1em;  background-color:#F16412 ;color:#fff;border-radius: 0.3em;margin-left:1.5em;"  onclick="del_posts(\''+v.pid+'\',\''+v.tid+'\',\''+v.userid+'\')" class="|hide|" value="清除">';
                    }
                    postshtml+='</p>';
                })
                /*<div class="ub ub-ver infloor" style="margin-top:1.5em;">
...
//删除
function del_threads(theoryIds,uid) {
    appcan.window.alert({
        title : "提示",
        content : '确定清除主题内容吗？',
        buttons : ['确定', '取消'],
        callback : function(err, data, dataType, optId) {
            ...
            if ("1" == data) {
                ...
                if(userInfo.id != '9d0d22aba39848ce84966f8faad4b192') uid=userInfo.id;
                ...
                var ferr=function(err){};
                var fok=function(data){
                    AddLogContent(userInfo.id, logKeys.ClubClearThread, {"threadId":theoryIds});
...
//删除
function del_posts(pid, tid, uid) {
    appcan.window.alert({
        title : "提示",
        content : '确定清除回复内容吗？',
        buttons : ['确定', '取消'],
        callback : function(err, data, dataType, optId) {
            if ("1" == data) {
                ...
                var fok=function(data){
                    AddLogContent(userInfo.id, logKeys.ClubClearReply, {"threadId":tid, "postId": pid});
