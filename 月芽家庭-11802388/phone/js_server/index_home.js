...
appcan.ready(function() {
    ...
    appcan.window.subscribe(UPDATE_INDEX, LoadAll);
...
appcan.button("#myreward", "btn-act", function() {
    appcan.window.open("user_reward_list", "user/user_reward_lists.html", 5);
});
appcan.button("#myRank", "btn-act", function() {
    appcan.window.open("rank_list", "ranking/ranking_lists.html", 5);
});
...
appcan.button("#introduction", "btn-act", function() {
    appcan.window.open("introduction", "book/book_introduct.html", 5);
});
//名师课程更多
appcan.button("#Famousteache", "btn-act", function() {
    appcan.window.open("book_introduction", "book/book_Famousteache.html", 5);
});
...
function LoadAll() {
    ...
    /*读数据库版本*/
    var type="普通会员";
    if(userInfo.isActiveValidity==1){
        type="VIP会员";
    }
    if(userInfo.type==2){
        type="月芽学生";
    ...
    $("#name").html('<a style="cusor:pointer; text-decoration: underline;" onclick="openUserInfo();">' + userInfo.name+' '+ type +'</a> ');
    var time1 = new Date().format("HH:mm:ss");
...
function LoadBooks() {
    ...
    var par={"params" : JSON.stringify({"userId":userInfo.id,"pageIndex":1, "pageSize":10})};
    var url = _SERVER_ADDRESS + "/phone/student/book/getRecommendBook";
    var ferr = function(err) {
    };
...
function LoadSwiper(data) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        objItem.bind("click", function() {
            ...
            appcan.window.open("pk_list", "pk_list.html", 10);
        });
...
function openUserSpace(){
    appcan.window.open("space_index", "space_index.html", 5);
}