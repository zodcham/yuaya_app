...
appcan.ready(function() {
    //userInfo.isBuyPPT = false;
    //userInfo.isBuyVideo = false;
    //setUserInfo(false);
...
appcan.button(".btnBuyPPT", "btn-act", function() {
    var orderObj={"goodsID":bookInfo.id, "money":1, "goodsName":bookInfo.name, "butType":"PPT"};
    setLocVal("orderObj",JSON.stringify(orderObj));
    appcan.window.open("order", "../vip/order.html", 5);
});
appcan.button(".btnBuyVideo", "btn-act", function() {
    var orderObj={"goodsID":bookInfo.id, "money":1, "goodsName":bookInfo.name, "butType":"Video"};
    setLocVal("orderObj",JSON.stringify(orderObj));
    appcan.window.open("order", "../vip/order.html", 5);
});
...
$(".preview").on("click", function() {
    ...
    //         content: '<div class="ub ub-ver" style="background-color: #fff;"><div class="ub-f1 tx-l" style="height:3em;line-height: 3em;">您还不是VIP会员，无法观看。您是否愿意成为我们的VIP会员？</div>' +
    //             '</div>'
    //         , btn: ['是的', '取消']
    //         , yes: function (index) {
    //             appcan.window.open("user_info", "user/user_info.html", 5);
...
function openBrowser(dataArrax) {
    ...
    if(userInfo.isActiveValidity==0 && userInfo.type!=2) {
        appcan.alert({"title": "提示", "content": "您是普通会员，仅可看20%的图片。"});
        if(dataArrax.length==0) return false;

        var max= parseInt(dataArrax.length*0.2);
        for(var i=0;i<max;i++){
            picList.push(dataArrax[i]);
        }
    }
    else
    {
...
function loadVideo2() {
    ...
    */
    ...
    video_player.onPlay(function(event){
        if(userInfo.isActiveValidity==0 && userInfo.type!=2){
            toast("您是普通会员，可观看50%的时间。", config.toastTimeLong);
        }
        if(timestart_video==0) {
            timestart_video=new Date().getTime(); //开始记录播放视频的时间
        }
    ...
    video_player.onTime(function(event){
        var pos=video_player.getPosition();
        if(userInfo.isActiveValidity==0 && userInfo.type!=2) {
            if (pos >= video_player.getDuration()/2) {
                toast("您是普通会员，可观看50%的时间。", config.toastTimeLong);
                video_player.stop();
