    ...
    */
    ...
    uexDevice.onOrientationChange = function(mode){
        if(mode==2){
            ...
            appcan.window.open("tv_index", "tv/tv_index.html", 5, 4);
        }
    };
...
function openVideByFilename(filename){
    closePlayer();
    var url="/sdcard/yueya/video/"+filename+".mp4";
    g_videoIndex=g_videoArr.indexOf(url);
    openPlayer();
}
...
function LoadVideoImageList() {
    ...
    for (var i = 0; i < g_ImageArr.length; i++) {
        ...
        bookItem.find(".bookImage").attr("filename", g_ImageFileNameArr[i]);
        bookItem.find(".bookName").html(g_ImageFileNameArr[i]);

        bookItem.bind("click", function() {
            //openNextVideo();
            var filename = $(this).find(".bookImage").attr("filename");
            openVideByFilename(filename);
        });
