...
appcan.ready(function() {
    ...
    uexWeiXin.cbShareLinkContent = function(data) {
        if (data == "0") {
            if(isShareWeixin){
                AddLogContent(userInfo.id, logKeys.SharingToWeixinSuccess, {"programId": programInfo.id});
            }
            else{
                AddLogContent(userInfo.id, logKeys.SharingToWeixinQuanSuccess, {"programId": programInfo.id});
            }
            appcan.window.alert({
                title : '提示信息！',
                content : '分享成功~',
            ...
            if(isShareWeixin){
                AddLogContent(userInfo.id, logKeys.SharingToWeixinFailed, {"programId": programInfo.id});
            }
            else{
                AddLogContent(userInfo.id, logKeys.SharingToWeixinQuanFailed, {"programId": programInfo.id});
            }
            appcan.window.alert({
                title : '提示信息！',
                content : '分享失败~',
...
function loadData() {
    ...
    getSysInfo();
    loadDatas();
    AddLogContent(userInfo.id, logKeys.SharingDetail, {"programId": programInfo.id});
    //用户名
...
function addLike(reviewId) {
    ...
    common.ajax("/programReview/addLikes", {
        ...
        AddLogContent(userInfo.id, logKeys.SharingReviewLike, {"programReviewId": reviewId});
...
var isShareWeixin=true;
appcan.button("#wedpageUrl0", "btn-act", function() {
    isShareWeixin=true;
    $("#divAddImg").addClass("uhide");
    var jsonstr0 = '{"thumbImg":"http://www.readseed.cn/yueya_webapp/icon.png","wedpageUrl":"http://www.readseed.cn/yueya_webapp/yueduquan/share_index_content.html?' + programInfo.id + '?","scene":"0","title":"' + programInfo.name + '","description":"分享一下"}';
...
appcan.button("#wedpageUrl1", "btn-act", function() {
    isShareWeixin=false;
    $("#divAddImg").addClass("uhide");
    var jsonstr1 = '{"thumbImg":"http://www.readseed.cn/yueya_webapp/icon.png","wedpageUrl":"http://www.readseed.cn/yueya_webapp/yueduquan/share_index_content.html?' + programInfo.id + '?","scene":"1","title":"' + programInfo.name + '","description":"分享一下"}';
