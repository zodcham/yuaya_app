...
appcan.ready(function(type) {
	...
	*/
    ...
    uexWidget.onSuspend = function () {
        //console.log('uexWidget.onSuspend'); //程序挂起的监听方法
        //记录学生关闭APP
    };
    uexWidget.onResume = function () {
        //console.log('uexWidget.onResume'); //程序恢复的监听方法
        //记录学生恢复APP
    };
...
function init() {
	...
	/*
    window.onorientationchange = window.onresize = function(mode) {
		alert(JSON.stringify(mode));
        appcan.window.open("root", "../root.html", 5, 4);
    };
	*/
    ...
    $(".tabs .tabItem").on("click", function() {
        ...
        case "video":
            ...
            break;
        case "audio":
            appcan.window.selectMultiPopover("content", 4);
            changeTab(4);
            break;
        }
    });
            ...
            appcan.window.alert({
                ...
                callback : function(err, data, dataType, optId) {
                    if (Number(data) != 1) {
                        ...
                        appcan.window.open("survey_detail", "survey_detail.html", 5);
                    } else {
                    }
                }
            });
...
function changeTab(index) {
    ...
    switch (index) {
        case 0:
            $("#home .tabItem").addClass("tab-active");
            break;
        case 1:
            $("#mall .tabItem").addClass("tab-active");
            break;
        case 2:
            $("#ppt .tabItem").addClass("tab-active");
            break;
        case 3:
            $("#video .tabItem").addClass("tab-active");
            break;
        case 4:
            $("#audio .tabItem").addClass("tab-active");
            break;
        /*case 3:
            AddLog(userInfo.id, logKeys.PersonEnter);
            break;*/
    }
}
...
function onReceiveNotificationOpen(info) {
    setLocVal("pushinfo",JSON.stringify(info));
    appcan.window.open("push", "push.html", 5);
}
