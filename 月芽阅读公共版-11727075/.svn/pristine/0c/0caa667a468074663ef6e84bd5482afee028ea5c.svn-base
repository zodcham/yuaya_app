...
function init() {
    ...
    if (programInfo.weixingLink.length > 0) {
        ...
        linkHtml = 'column_index_introduce_content.html'+GetUrlRootPar();
    }

    appcan.frame.open({
        id : "content",
        url : [{
            'inPageName' : 'column_index_introduce_content',
            'inUrl' : linkHtml
        }, {
            'inPageName' : 'column_index_comments_content',
            'inUrl' : 'column_index_comments_content.html'+GetUrlRootPar()
        }, {
            'inPageName' : 'column_index_content',
            'inUrl' : 'column_index_content.html'+GetUrlRootPar()
        }],
        top : titHeight,
        left : 0,
        index : 0,
...
function exit() {
    //uexWindow.evaluatePopoverScript("", "introduce", "closeAudio()");
    //appcan.window.evaluatePopoverScript("", "introduce", "closeAudio()");
