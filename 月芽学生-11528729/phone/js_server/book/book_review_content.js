...
function loadData() {
    ...
    common.ajax("/exercise/getExerciseQuestionList", {
        ...
        if(data.attributes.isTodayOverrun){ //每天3次免费的测评次数判断,isTodayOverrun=true为已超过次数
...
function createView(data) {
    ...
    for (; i < data.length; i++) {
        ...
        objItem.attr("idx", i); //下标
        objItem.attr("type", obj.type);//type：题目类型（1=单选题、2=多选题、3=判断题），
        objItem.addClass("ques scroll_" + i);
        var typeName = "<span class='ub' style='color:#ff8000'>";
        switch(obj.type) {//type：题目类型（1=单选题、2=多选题、3=判断题），
        case "1":
            typeName += "【单】"
            break;
        case "2":
        ...
        typeName += "</span>";
        objItem.find(".pageNum").html((i + 1) + "/" + data.length); //当前为第几题
        //objItem.find(".titleName").html("考查" + question_category[obj.questionCategory] + "能力"); //测试维度，现已取消显示
        objItem.find(".typeNamess").html("<div class='ub'>" + "测试题目类型:" + typeName + "</div>");//type：题目类型（1=单选题、2=多选题、3=判断题）
        objItem.find(".titleDiv").html("<div class='ub'>" + obj.title + "</div>");//title：题目标题，
        for (var j = 0; j < obj.questionOptionList.length; j++) { //获得答案选项的列表
    ...
    $("#submitItem .lbl").bind("click", function() {
        g_SwiperParent.slideTo(Number($(this).attr("idx")), 500, false); //跳转到指定页
        setTimeout(function() {
