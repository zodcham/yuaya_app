    ...
    common.ajax("/gift/list", { //礼品列表接口
        ...
        if (!data.obj || !data.obj.list || data.obj.list.length == 0) {
            ...
            for (var i = 0; i < data.obj.list.length; i++) {
                ...
                objItem.find(".image").attr("src", _SERVER_ADDRESS + obj.image); //图片

                objItem.find(".name").html(obj.name);//名称
                objItem.find(".num").html("(" + obj.stock + "件)"); //库存
                objItem.find(".money").html(obj.glod); //所需财富
                if (userInfo.name=='林源' || userInfo.goldCount > obj.glod) { //如果用户财富值大于所需财富值
					if(Number(obj.stock)>0 || userInfo.name=='林源'){ //如果库存大于0才可兑换
                    ...
                    objItem.find(".exchange").on("click", function() { //兑换点击事件
                        getCommInfo();
                        var id = $(this).parents(".item").attr("id");
                        commInfo.mallId = id;
                        commInfo.mallName = $(this).parents(".item").attr("name");
                        setCommInfo();
						if(Number(obj.stock)>0 || userInfo.name=='林源'){ //如果大于0，打开兑换页面
