document.write("<script src='http://www.readseed.cn/nocache/js_student/json/index_home_json.js?ver=" + Math.random() + "'></script>");
//获得首页固态JSON
...
function LoadAll() {
    ...
    var fok = function(data) {
        ...
        appcan.logs(JSON.stringify(data.lbsj));
...
function loadBanner(lbsj){
    ...
    common.ajax("/activity/page", {
        ...
        if(data.obj.hasOwnProperty("sysList")) {//如果所属平台有活动且在活动期间，显示平台活动。
            list = data.obj.sysList.list;
            if(list){
                for(var i=0;i<list.length;i++) {
                    var obj = list[i];
