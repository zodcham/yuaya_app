...
appcan.ready(function() {
    ...
    $("#txtNickname").val(userInfo.nickName);
    $("#txtGrade").val(userInfo.gradeName);


    var size = $("#divImg").width() * 0.19;
...
function doSubmit() {
    ...
    if (title.length == 0) {
        toast("请输入标题！", config.toastTimeShort);
        return;
    }

    var content = $("#txtComments").val();
    if (content.length == 0) {
        toast("请输入内容！", config.toastTimeShort);
        return;
    }
    var name = $("#txtName").val();
    if (name.length == 0) {
        toast("请输入姓名！", config.toastTimeShort);
        return;
    }
    var mobile = $("#txtMobile").val();
    if (mobile.length == 0) {
        toast("请输入手机！", config.toastTimeShort);
        return;
    }
    var nickname = $("#txtNickname").val();
    if (nickname.length == 0) {
        toast("请输入笔名！", config.toastTimeShort);
        return;
    }
    var checkNum = /[\ud800-\udbff][\udc00-\udfff]/g;
    ...
    var params = {
        ...
        'participantPhone' : mobile,
        'participantGrade' : grade,
        'participantPen' : nickname,
        'participantAddress' : address
    };
    common.ajax("/activity/production/add", {
...
function upload(imgPath, i) {
    ...
    var params = "?productionId=" + reviewId + "&imageIndex=" + (i + 1) + "&imageCount=" + imgList.length + "&type=1";
    var uploadHttp =  _SERVER_ADDRESS+"/phone/activity/production/addImage"+params;
