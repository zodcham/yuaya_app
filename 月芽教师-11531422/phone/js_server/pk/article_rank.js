...
appcan.ready(function() {
	$("body").removeClass("sign_detai_bg");
    ...
    var fok = function(data) {
        ...
        if (g_page == 1) {
            for (var i = 0; i < 3; i++) {
                ...
                if (arr[i]) {
                    var time = arr[i].neverTotal;
                    $("#top3_" + i).html(GetName(arr[i]) + " " + time)

                    var img = _SERVER_ADDRESS + arr[i].studentPhoto;
                    $("#top3_img_" + i).attr("src", img);
					
					var rank=i+1;
					$("#rank"+rank).parent().attr("articleId",arr[i].id);
					$("#rank"+rank).parent().bind("click",function(){
						openArticle($(this).attr("articleId"));
					});
...
function GetListHtml(obj) {
    ...
    html += '     <div class="sign_list_num">' + obj.rankIndex	 + '</div>';
