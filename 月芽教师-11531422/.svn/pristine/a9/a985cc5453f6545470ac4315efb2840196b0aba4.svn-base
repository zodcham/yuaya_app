...
function loadDic() {
    ...
    common.ajax("/dict/getDictList", {
        ...
        AddLogContent(commInfo.taskData.userId, logKeys.TestStart,{"bookId":commInfo.taskData.bookId});
...
function doSubmit(quesIds, optionIds) {
    stopTimer();
    var params = {
        'userId' : commInfo.taskData.userId,
        'bookId' : commInfo.taskData.bookId,
        'questionIds' : quesIds,
        'optionIds' : optionIds
    };
    common.ajax("/exercise/saveExercise", {
        ...
        AddLogContent(commInfo.taskData.userId, logKeys.TestEnd,{"bookId":commInfo.taskData.bookId, "timecount":timecount_test});
        test_already=1;
        ...
        var html='';
        if(test_count<=3){
            html='<div class="ub ub-ac ub-pc umar-t " style="color:#fff;font-size:0.9em;">通过这次测评，您获得 '+ add_gold +' 财富 和 '+ add_score +' 积分。</div>;';
        }
        else{
            html='<div class="ub ub-ac ub-pc umar-t " style="color:#fff;font-size:0.9em;">您此次测评是第 '+ test_count +'次，已扣除 10 财富。<br>说明: 如果测评超过3次，每次将扣除10财富。</div>;';
        }
        //$(".accuracy-box").append(html);


        appcan.window.evaluateScript({
            name : "user_task_details", //任务完成情况页刷新
            scriptContent : 'LoadPage();'
        });
        appcan.window.evaluateScript({
            name : "user_task_help", //辅助测评页刷新
            scriptContent : 'LoadPage();'
        });
...
function loadData() {
    ...
    common.ajax("/exercise/getExerciseQuestionList", {
        ...
        appcan.window.evaluateScript({
            name : "user_task_help_review", //窗口名称
...
appcan.button("#btnClose", "btn-act", function() {
    appcan.window.evaluateScript({
        name : "user_task_help_review", //窗口名称
        scriptContent : 'appcan.window.close(5);'
    });
